...
const MakeCourse = () => {
  ...
  useEffect(() => {
    if (map !== null) {
      ...
      setValues({ ...values, waypoint: linePath, center: center, level: lv, distance: calculateDist() });
    }
  ...
  return (
    <div className="make-course relative">
      ...
      {/* 카카오 맵 */}
      <div id="map" className="relative" style={{ width: '400px', height: '900px' }}></div>
      ...
      <div className="absolute flex items-center justify-center p-2 bg-white rounded-full border border-gray-200 shadow-lg bottom-[150px] left-3 z-10">
        <span className="text-xl font-bold text-[#232323]">{calculateDist()}<span className="text-base text-[#888888]">KM</span></span>
      </div>

  
      {/* 코스 등록 상세 페이지 */}
      <button className="absolute w-full flex items-center justify-center p-2 bottom-[86px] z-10" onClick={() => toggleMakeCoursePop()} >
      ...
      <div className={`makeCoursePop flex flex-col bg-slate-600 text-lg z-50`} onSubmit={handlePostCourse} >
      ...
      <div className="flex flex-col p-4 ">
        ...
        <form className="flex flex-col gap-2 py-2 text-black" > 
          ...
          {/* 이미지 업로드 */}
          <img className="w-full" src = {uploadImgUrl} alt="Preview" img = "img"/>
