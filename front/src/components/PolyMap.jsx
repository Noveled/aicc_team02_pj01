...
const PolyMap = () => {
  ...
  const handleGetCourse = (e) => {
    ...
    axios.get('http://localhost:8080/get_course?isVisible=true')
    .then((res) => {
      // console.log(res);
      if (res.status === 200) {
        // navigate('/login');
  ...
  const handlePostCourse = (e) => {
    ...
    if(!values.course_name || !values.user_id || !values.distance || !values.city || !values.url) {
      alert('입력값을 확인해주세요.');
      return;
    }
  ...
  return (
    <>
      ...
      <form className='flex gap-2 py-2' onSubmit={handlePostCourse}>
        ...
        <input className='border border-gray-400 rounded-md' 
        type="text" name="content" placeholder='코스 설명'
        ...
        <input className='border border-gray-400 rounded-md' 
        type="text" name="city" placeholder='지역명 입력(ex.구로구)' 
        onChange={(e) => setValues({ ...values, city: e.target.value })} />
        비밀글 여부 : 
        <input className='border border-gray-400 rounded-md' 
        type="checkbox" name="is_private"
        onChange={(e) => setValues({ ...values, is_private: e.target.checked })} />
        <input className='border border-gray-400 rounded-md' 
        type="text" name="url" placeholder='이미지 URL' 
      ...
      <button className='border border-gray-400 rounded-md p-1' 
      onClick={getMapInfo}>맵 정보 구하기</button>
      {/* <button className='border border-gray-400 rounded-md p-1'  */}
